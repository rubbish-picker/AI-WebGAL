...
export const scriptExecutor = () => {
  webgalStore.dispatch(setStage({key:'aiFullShowText',value:""}));
  webgalStore.dispatch(setStage({key:"aiShowTextUUIDForSentence",value:""}));
  webgalStore.dispatch(setStage({key:"aiShowTextUUIDForParagraph",value:""}));
  //如果有ai回复，则先显示ai回复
  const AIState = webgalStore.getState().AI;
  var NeedDealAiResponse = false;
  if (AIState.sentenceGenerated.length > 0) {
    NeedDealAiResponse = true;
  }
  ...
  //modify start 分情况处理，并进行深复制防止只读错误
  var originalScript: ISentence;
  if(NeedDealAiResponse) {
    originalScript=
     webgalStore.getState().AI.sentenceGenerated[0].sentence;
  }
  else{
    originalScript=
    WebGAL.sceneManager.sceneData.currentScene.sentenceList[WebGAL.sceneManager.sceneData.currentSentenceId];
  }
  const currentScript: ISentence = cloneDeep(originalScript);
  //modify end
  ...
  if (!runThis) {
    ...
    if(!NeedDealAiResponse) WebGAL.sceneManager.sceneData.currentSentenceId++;
    nextSentence();
    return;
  }
  runScript(currentScript);
  if(NeedDealAiResponse) webgalStore.dispatch(AiActions.removeGeneratedSentence(0));
