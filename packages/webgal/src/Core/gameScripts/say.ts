...
import { AiActions } from '@/llm/AIReducer';
import { AIres } from '@/llm/AIFeedback';
import { generateUUID } from '@/llm/utils';
import { IAiSentenceState } from '@/llm/AIInterface';
import { aiConfig } from '@/llm/AIConfig';
...
export const say = (sentence: ISentence): IPerform => {
  ...
  const isAIChat = getSentenceArgByKey(sentence, 'aichat') as boolean; // 是否是AI对话
  const isAIexp = getSentenceArgByKey(sentence, 'aiexp') as boolean; // 是否是AI表情
  const isAIbg = getSentenceArgByKey(sentence, 'aibg') as boolean;//是否是AI背景
  const isAIbgm = getSentenceArgByKey(sentence, 'aibgm') as boolean;//是否是AI音乐
  const isAIscene = getSentenceArgByKey(sentence, 'aiscene') as boolean;//是否是AI场景
  const AiName = getSentenceArgByKey(sentence, 'ainame') as string; // AI该次对话名称
  const AiShowAfterPoint = getSentenceArgByKey(sentence, 'needpoint') as boolean;//是否是点击后才显示AI文本
  const AiFullShowText = getSentenceArgByKey(sentence, "AiFullShowText") as string;//AI全文本
  const AiShowTextUUIDForSentence = getSentenceArgByKey(sentence, "AiShowTextUUIDForSentence") as string;//AI全文本UUID
  const AiShowTextUUIDForParagraph = getSentenceArgByKey(sentence, "AiShowTextUUIDForParagraph") as string;//AI全文本UUID


  if (AiShowTextUUIDForSentence != "" && AiShowTextUUIDForSentence != null && AiShowTextUUIDForSentence != null) {
    webgalStore.dispatch(setStage({ key: "aiFullShowText", value: AiFullShowText }));
    webgalStore.dispatch(setStage({ key: "aiShowTextUUIDForSentence", value: AiShowTextUUIDForSentence }));
    webgalStore.dispatch(setStage({ key: "aiShowTextUUIDForParagraph", value: AiShowTextUUIDForParagraph }));
    //提取参数后设置为舞台状态，方便backlog保存
  }

  //如果是AI反馈，那么显示的内容应该是ai回复的等待词
  if (isAIChat) {
    let uuid = generateUUID();
    let AiSentenceStateToAdd: IAiSentenceState = {
      id: uuid,
      name: AiName,
      prompt: dialogToShow,
      response: '',
      isLoading: true,
      isError: false,
      forDialogue: isAIChat,
      forFigure: isAIexp,
      forBackground: isAIbg,
      forMusic: isAIbgm,
      forScene: isAIscene,
      model: aiConfig.model,
      showAfterPoint: AiShowAfterPoint,
    }
    webgalStore.dispatch(AiActions.addAiSentence(AiSentenceStateToAdd));
    AIres(AiSentenceStateToAdd);
    logger.info("New Aichat Request Added",webgalStore.getState().AI.OnGoings);
    
    dialogToShow = aiConfig.waitingInfo;
  }
