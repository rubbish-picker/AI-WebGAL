...
import { cardManager } from '@/llm/cardManager';
import { forEach } from 'lodash';
import { parsedCardManager } from '@/llm/cardParser';
...
function getAndParseAIInfo(){
  axios.get('./game/AIconfig/cardTable.json').then((res) => {
    const cards:Array<string> = res.data;
    for (const cardName of cards) {
      axios.get(`./game/AIconfig/${cardName}.json`).then((res) => {
        if (res.data) {
          cardManager.addCard(cardName, res.data);
          parsedCardManager.parseAndAdd(cardName, res.data);
        }
      });
    }
  });
